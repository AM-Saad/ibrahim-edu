...
(function () {
  const examConfig = {
    ...
    startExam: async function () {
      const data = await fetchdata(`/exam/start/${this.examId}?lessonId=${this.lessonId}`, 'Post', {}, false)
      if (data != null) {
    ...
    confirmAnswer: async function (info) {
      ...
      const data = await fetchdata(`/exam/answer/${info.lessonId}?qid=${info.questionId}`, 'put', newform, false)
      if (data != null) {
    ...
    submitExam: async function (e) {
      ...
      try {
        let r = await fetch(`/exam/submit/${lessonId}`, {
          method: "POST",
          body: examForm,
        });
