  ...
  exam.lessonQuestions.forEach((q, i) => {
    ...
    $(".pagination-list").append(`
          <li class="pagination-list_item" >
            <button data-scroll="${
              conunter
            }" class="questionNumber btn btn-info">${conunter}</button>
          </li>
      `);
    if (q.questionType === "choose") {
      $(".wrapper #exam").append(`
        ...
        <div class="answer p-3 text-sm shadow-sm flex items-center gap-3 rounded bg-white mb-2" style="display:none;">
        <input type="checkbox" name="selectedAnswers" value="${q.answers[0].answerNo}" />
        ${q.answers[0].answer}
        </div>
        <div class="answer p-3 text-sm shadow-sm flex items-center gap-3 rounded bg-white mb-2">
      ...
      <div id="${conunter}" class="form-group exam-question">
          ...
          <div class="writeAnswer" data-target="${q._id}">
            <div class="canvaswrapper">
              ...
              <button type="button" class="clearCanvas btn btn-danger">Clear</button>
            </div>
            <a id="startAnswer" class="startAnswer btn btn-info">
              ابدأ الإجابة
            </a>
        </div>
      </div>
